...
\begin{lstlisting}[language=Python, caption= {Aplicación Web básica creada con Flask. Se crea una API que devolverá la cadena de texto \textit{Hello world!} al ser invocada. Esto se implementa para ver que la librería está instalada correctamente y funciona. Fuente: Wikipedia}]
...
\begin{lstlisting}[language=Python, label = {lst:API}, caption={Creación de la API del sistema de recomendación en Python usando Flask. En primer lugar se eliminan los posibles webhooks que existieran y se crea el nuevo con la URL a través de la cual pueda accederse a la API que estamos construyendo. En la función \texttt{main}, que es la que procesará los mensajes, se filtra para procesar únicamente los mensajes que lleguen tras el encendido. Esto se hace porque en caso contrario se procesarían de golpe todos los mensajes que hubieran llegado con la API apagada. Se instancia el recomendador para poder pedirle recomendaciones. En caso de que el mensaje sea /start significará que es el inicio de la conversacion con el bot y se saluda al usuario. En caso contrario , se intenta recomendar películas con el mensaje que se haya recibido, dando por hecho que será el título de una película. Finalmente, se generan las reocomendaciones, se maqueta el mensaje que se enviará al usuario y se hace una llamada a la API de Telegram para que envíe el mensaje con la respuesta al chat de origen.}]
import requests
import time
import math
import os, json
...
\begin{lstlisting}[language=Python, label = {lst:API}, caption= {Búsqueda del id de la película dada por el usuario. Es muy posible que el usuario no escriba de forma correcta completamente el título de la película. Estas funciones implementan la búsqueda de la película más parecida dentro de un umbral. El objetivo es transformar una cadena de texto de entrada en un número de fila del dataframe de películas. Es decir, buscan con qué id de película se corresponde ese texto. Para ello, se hace uso de la distancia Levenshtein, de forma que podemos definir la distancia entre dos cadenas de texto. La película cuyo título se parezca más al dado por el usuario (siempre que la distancia sea menor a un umbral) será la que se use de referencia a la hora de hacer recomendaciones.}]
    @staticmethod
